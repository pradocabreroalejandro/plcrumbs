CREATE OR REPLACE PACKAGE BODY pkg_solar_energy AS
  ...
  BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('>>> ' || 'get_panel_strings' || ' p_installation_id=' || p_installation_id);
    ...
    PLTelemetry.attr('l_panels_per_str', l_panels_per_str);
    DBMS_OUTPUT.PUT_LINE('l_tot_panels=' || l_tot_panels);
    DBMS_OUTPUT.PUT_LINE('l_panel_voc=' || l_panel_voc);
    DBMS_OUTPUT.PUT_LINE('l_panel_isc=' || l_panel_isc);
    DBMS_OUTPUT.PUT_LINE('l_panels_per_str=' || l_panels_per_str);
    ...
    DBMS_OUTPUT.PUT_LINE('l_string_voc=' || l_string_voc);
    l_string_power := l_string_voc * l_panel_isc;        -- [LOG:debug]
    PLTelemetry.attr('l_string_power', l_string_power);
    DBMS_OUTPUT.PUT_LINE('l_string_power=' || l_string_power);
    ...
    DBMS_OUTPUT.PUT_LINE('<<< get_panel_strings');
  ...
  BEGIN
    -- [LOG:trace]
    l_span_id := PLTelemetry.start_span('get_string_health');
    PLTelemetry.attr('p_installation_id', p_installation_id);
    PLTelemetry.attr('p_string_id', p_string_id);
    DBMS_OUTPUT.PUT_LINE('>>> ' || 'get_string_health' || ' p_installation_id=' || p_installation_id || ' p_string_id=' || p_string_id);
    ...
    PLTelemetry.attr('l_expected_voc', l_expected_voc);
    PLTelemetry.attr('l_measured_voc', l_measured_voc);
    DBMS_OUTPUT.PUT_LINE('l_expected_voc=' || l_expected_voc);
    DBMS_OUTPUT.PUT_LINE('l_measured_voc=' || l_measured_voc);

    l_deviation_pct := ABS(l_measured_voc - l_expected_voc) / l_expected_voc * 100; -- [LOG:debug]
    PLTelemetry.attr('l_deviation_pct', l_deviation_pct);
    DBMS_OUTPUT.PUT_LINE('l_deviation_pct=' || l_deviation_pct);
    ...
    l_status := CASE
                  ...
                  ELSE                            'FAULT'
                END; -- [LOG:debug]
                PLTelemetry.attr('l_status', l_status);
                DBMS_OUTPUT.PUT_LINE('l_status=' || l_status);

    -- [LOG:trace]
    PLTelemetry.end_span('OK');
    DBMS_OUTPUT.PUT_LINE('<<< get_string_health');
    RETURN l_status;
